...
def randomize_rigid_object_com(
    ...
    com = asset.root_physx_view.get_coms().clone()  # (N, 7) for rigid object
    ranges = torch.tensor(
        [[*com_range.get("x", (0.0, 0.0))],
         [*com_range.get("y", (0.0, 0.0))],
         [*com_range.get("z", (0.0, 0.0))]], device="cpu"
    )
    rand = math_utils.sample_uniform(ranges[:, 0], ranges[:, 1], (len(env_ids), 3), device="cpu")
    com[env_ids, :3] += rand
    asset.root_physx_view.set_coms(com, env_ids)
...
def init_zero_joint_default_pos(
    ...
    asset.data.default_joint_pos[env_ids, joint_ids] = 0.0
...
def curriculum_gravity_with_randomization(
    env: DirectRLEnv,
    env_ids: torch.Tensor,
    gravity_stages: list[tuple[int, tuple[float, float, float], tuple[float, float, float]]],
) -> None:
    """
    Apply gravity curriculum based on training progress.

    Args:
        env: The environment instance
        env_ids: Environment IDs (not used for global gravity change)
        gravity_stages: List of (step_threshold, (gx_min, gy_min, gz_min), (gx_max, gy_max, gz_max)) tuples
    """
    from isaaclab.envs.mdp import randomize_physics_scene_gravity
    current_step = env.common_step_counter

    # Find the appropriate gravity based on training progress
    target_gravity_min = (0.0, 0.0, -9.81)  # default
    target_gravity_max = (0.0, 0.0, -9.81)  # default
