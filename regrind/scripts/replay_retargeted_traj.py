...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg, agent_cfg):
    ...
    def _sync_manager_hand(timestep: int, *, open_loop_pd: bool) -> None:
        ...
        fingers = a[:, 6:]
        hand_asset.write_root_link_pose_to_sim_index(root_pose=root_state[:, :7])
        hand_asset.write_root_link_velocity_to_sim_index(root_velocity=root_state[:, 7:])
        if open_loop_pd:
    ...
    def _write_demo_object_state(timestep: int) -> None:
        ...
        object_asset.write_root_link_pose_to_sim_index(root_pose=object_root_state[:, :7])
        object_asset.write_root_link_velocity_to_sim_index(root_velocity=object_root_state[:, 7:])
        if hasattr(object_asset, "num_joints") and object_asset.num_joints > 0:
    ...
    def _step_open_loop_dynamics(timestep: int):
        ...
        if args_cli.step_dynamics:
            ...
            if args_cli.replay_debug and (
                ...
                reached = hand_asset.data.joint_pos.torch[0, hand_joint_ids]
