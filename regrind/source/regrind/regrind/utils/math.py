...
def quat_from_euler_xyz_intrinsic(roll: torch.Tensor, pitch: torch.Tensor, yaw: torch.Tensor) -> torch.Tensor:
    """Convert intrinsic XYZ (Tait-Bryan) Euler angles in radians to unit quaternions ``(x, y, z, w)``.
    ...
    # Closed form for q_x * q_y * q_z (Hamilton); matches scipy Rotation.from_euler("XYZ", ...).
