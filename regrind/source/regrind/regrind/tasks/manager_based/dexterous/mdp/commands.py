...
class MotionLoader:
    ...
    def _load_retargeted_traj(self, retargeted_traj_path: str, speed_factor: float):
        ...
        hand_wrist_euler_XYZ = R.from_quat(hand_wrist_quat).as_euler("XYZ", degrees=False)
    ...
    def _get_object_keypoints_pos(
        ...
        if root_quat.ndim == 1:
            root_quat = root_quat[None, ...]  # (B,4) XYZW

        # Bottom rotation (world-from-bottom), applied per batch
        Rb = R.from_quat(root_quat)  # (B,) rotations
        B = root_pos.shape[0]
...
class MotionCommand(CommandTerm):
    ...
    def __init__(self, cfg: MotionCommandCfg, env: ManagerBasedRLEnv):
        ...
        self.object: BaseArticulation = env.scene[cfg.object_asset_name]
        ...
        self.motion: MotionLoader = MotionLoader(
            ...
            is_articulated_object=isinstance(self.object, BaseArticulation),
            device=self.device
        )
    ...
    def _finite_difference_quat(self, quat_fn, time_steps: torch.Tensor) -> torch.Tensor:
        ...
        quat_diff = torch.where(quat_diff[:, 3:4] < 0.0, -quat_diff, quat_diff)
    ...
    def current_object_joint(self) -> torch.Tensor | None:
        if isinstance(self.object, BaseArticulation):
            return self.object.data.joint_pos.torch
        else:
            return None
    ...
    def current_hand_wrist_pos(self) -> torch.Tensor:
        return self.robot.data.root_pos_w.torch - self._env.scene.env_origins
    ...
    def current_fingertips_pos(self) -> torch.Tensor:
        ...
        pos_w = self.tfs["tf_fingertips"].data.target_pos_w.torch
        return pos_w - self._env.scene.env_origins.unsqueeze(1)

    @property
    def current_hand_joint_pos(self) -> torch.Tensor:
        return self.robot.data.joint_pos.torch[:, self.actuated_dof_indices]

    @property
    def current_hand_joint_vel(self) -> torch.Tensor:
        return self.robot.data.joint_vel.torch[:, self.actuated_dof_indices]

    @property
    def default_hand_joint_pos(self) -> torch.Tensor:
        return self.robot.data.default_joint_pos.torch[:, self.actuated_dof_indices]
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        self.object.write_root_link_velocity_to_sim_index(
            root_velocity=torch.cat([object_lin_vel[env_ids], object_ang_vel[env_ids]], dim=-1),
            env_ids=env_ids
        )
        if isinstance(self.object, BaseArticulation):
        ...
        self.robot.write_root_link_pose_to_sim_index(
            root_pose=torch.cat(
                [hand_pos[env_ids] + self._env.scene.env_origins[env_ids], hand_quat[env_ids]],
                dim=-1,
            ),
            env_ids=env_ids,
        )
        self.robot.write_root_link_velocity_to_sim_index(
            root_velocity=torch.cat([hand_lin_vel[env_ids], hand_ang_vel[env_ids]], dim=-1),
            env_ids=env_ids,
        )
        ...
        soft_lo = soft_joint_pos_limits[env_ids][:, dof_ids, 0]
        soft_hi = soft_joint_pos_limits[env_ids][:, dof_ids, 1]
    ...
    def _debug_vis_callback(self, event):
        ...
        if self.object_keypoint_markers is not None:
            ...
            default_quat = torch.tensor([0.0, 0.0, 0.0, 1.0], device=self.device).repeat(
                2 * flat_count, 1
            )
        ...
        if self.cfg.debug_vis_contact_force_arrows and "contact_forces" in self._env.scene.sensors:
            ...
            forces = cs.data.net_forces_w.torch
