...
class ClippedRelativeJointPositionAction(joint_actions.RelativeJointPositionAction):
    ...
    def __init__(self, cfg: "ClippedRelativeJointPositionActionCfg", env: ManagerBasedEnv):
        ...
        # Isaac Lab 3.x: articulation data is Warp-first.
        # Explicitly request the Torch view for PyTorch advanced indexing.
        joint_pos_limits = self._asset.data.soft_joint_pos_limits.torch
    ...
    def get_base_joint_pos(self) -> torch.Tensor:
        ...
        if self.cfg.base_action_source == "current_obs":
            ...
            return self._asset.data.joint_pos.torch[:, self._joint_ids]  # TODO: consistency with observations
...
def _quat_positive_real(q: torch.Tensor) -> torch.Tensor:
    """Map XYZW quaternion to the hemisphere with nonnegative real part (w)."""
    return torch.where(q[..., 3:4] < 0, -q, q)


def _rotvec_to_quat(rotvec: torch.Tensor) -> torch.Tensor:
    """Convert rotation vectors (axis-angle, radians) to quaternions (x, y, z, w)."""
...
class SE3ImpedanceActionTerm(ActionTerm):
    ...
    def __init__(self, cfg: SE3ImpedanceActionCfg, env: ManagerBasedEnv):
        ...
        self._wrench_body_ids_tensor = torch.tensor(
            self._wrench_body_ids, dtype=torch.int32, device=self.device
        )
        ...
        self._body_mass = self._asset.data.default_mass.torch.to(self.device)  # (num_envs, num_bodies)
    ...
    def get_base_pose(self) -> tuple[torch.Tensor, torch.Tensor]:
        """Base pose: env-relative position (matches motion command), world quaternion XYZW."""
    ...
    def _get_base_pose(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        pos = self._asset.data.root_pos_w.torch - self._env.scene.env_origins
        quat = self._asset.data.root_quat_w.torch
        if self.cfg.base_action_source == "current_obs":
    ...
    def _com_pos_w(self) -> torch.Tensor:
        body_com_w = self._asset.data.body_com_pos_w.torch          # (num_envs, num_bodies, 3)
    ...
    def _com_pos_b(self) -> torch.Tensor:
        ...
        root_pos_w = self._asset.data.root_link_pos_w.torch
        root_quat_w = self._asset.data.root_link_quat_w.torch
    ...
    def _root_com_pos_b(self) -> torch.Tensor:
        ...
        root_pos_w = self._asset.data.root_link_pos_w.torch
        root_quat_w = self._asset.data.root_link_quat_w.torch
        return quat_apply_inverse(root_quat_w, self._asset.data.root_com_pos_w.torch - root_pos_w)
    ...
    def apply_actions(self):
        ...
        pos = self._asset.data.root_pos_w.torch - self._env.scene.env_origins
        quat = self._asset.data.root_quat_w.torch
        root_link_vel = self._asset.data.root_link_vel_w.torch
        vel = root_link_vel[:, :3]
        omega = root_link_vel[:, 3:]
        ...
        # Preserve the original root force application. Only distribute torque to avoid exciting
        # Isaac Sim 6's low-inertia dummy root link.
        body_mass = self._asset.data.body_mass.torch
        torque_weights = body_mass / body_mass.sum(dim=1, keepdim=True)
        wrench_composer = self._asset.permanent_wrench_composer
        wrench_composer.set_forces_and_torques_index(
            forces=f.unsqueeze(1),
            torques=None,
            positions=None,
            body_ids=self._wrench_body_ids_tensor,
            env_ids=None,
            is_global=True,
        )
        wrench_composer.add_forces_and_torques_index(
            forces=None,
            torques=tau.unsqueeze(1) * torque_weights.unsqueeze(-1),
            positions=None,
            body_ids=None,
            env_ids=None,
            is_global=True,
